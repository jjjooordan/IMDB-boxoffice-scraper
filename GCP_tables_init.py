...
conn.execute(
    "CREATE TABLE db_actor (href_actor varchar PRIMARY KEY, fullname varchar NOT NULL, dob date)"
)
...
conn.execute(
    "CREATE TABLE db_film (href_film varchar PRIMARY KEY, title varchar NOT NULL, year integer, imdb_rating numeric, imdb_qty numeric, budget numeric, gross_wknd numeric, gross_domestic numeric, gross_ww numeric)"
)
...
conn.execute(
    "CREATE TABLE db_filmcredits (key_filmact varchar PRIMARY KEY, href_film varchar REFERENCES db_film, href_actor varchar REFERENCES db_actor)"
)
