# IMDB_filmo_upsert.py
# Function that upserts two IMDB_filmo_scraper.py DataFrames to Google Cloud Platform Cloud SQL server
# Ordered inputs are result_films DataFrame, and result_actor DataFrame
# Ref. IMDB_filmo_scraper.py for DataFrame details
# Current version has hardcoded GCP credentials
...
# Create function to upsert IMDB DataFrames
def imdb_filmo_upsert(result_films, result_actor):
    
    # Specify Google Cloud Platform parameters (HARDCODED)
    ...
    print("Google Cloud Platform connect success.")

    # Clear and create temp_table_film
    print("Clearing temp_table_film, if it exists.")
    conn.execute(
    ...
    print("Clearing temp_table_actor, if it exists.")
    conn.execute(
    ...
    print("Created temp_table_actor.")
